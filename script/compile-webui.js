...
function getRunWebUISh() {
  return [
    ...
    '  if command_exists pkg; then',
    '    run_as_root pkg install -y git gh git-lfs',
    '  elif command_exists apt-get; then',
    '    run_as_root apt-get update',
    '    run_as_root apt-get install -y git git-lfs gh || run_as_root apt-get install -y git git-lfs',
    '  elif command_exists dnf; then',
    '    run_as_root dnf install -y git git-lfs gh',
    '  elif command_exists yum; then',
    '    run_as_root yum install -y git git-lfs gh',
    '  elif command_exists pacman; then',
    '    run_as_root pacman -Sy --noconfirm git github-cli git-lfs',
    '  elif command_exists zypper; then',
    '    run_as_root zypper install -y git git-lfs gh',
    '  elif command_exists apk; then',
    '    run_as_root apk add git git-lfs github-cli',
    '  fi',
    '  hash -r 2>/dev/null || true',
    '}',
    '',
    ...
    '    install_git_tooling',
    '    if ! command_exists gh; then',
    '      echo "GitHub CLI (gh) was not found on PATH. Install gh before starting GitDesk WebUI." >&2',
    '      exit 1',
    '    fi',
    '  fi',
    '',
    ...
    '    install_git_tooling',
    '    if ! "$git_cmd" lfs version >/dev/null 2>&1; then',
    '      echo "git-lfs was not found or is not available through Git. Install Git LFS before starting GitDesk WebUI." >&2',
    '      exit 1',
    '    fi',
    '  fi',
    '}',
...
function getRunWebUIPowerShell() {
  return [
    ...
    '  if ($LASTEXITCODE -ne 0) {',
    '    Write-Warning "winget could not install $Name. The runtime check will report a hard error if it is still unavailable."',
    '  }',
    '',
    '  Refresh-Path',
    '}',
    ...
    '    $ghCommand = Get-Command gh -ErrorAction SilentlyContinue',
    '    if ($null -eq $ghCommand) {',
    '      throw "GitHub CLI (gh) was not found on PATH. Install gh before starting GitDesk WebUI."',
    '    }',
    '  }',
    '',
    ...
    '    & $gitCommandPath lfs version *> $null',
    '    if ($LASTEXITCODE -ne 0) {',
    '      throw "git-lfs was not found or is not available through Git. Install Git LFS before starting GitDesk WebUI."',
    '    }',
    '  }',
    '}',
    '',
